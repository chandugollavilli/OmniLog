...
class AIService {
    static async querySecurityAssistant(userPrompt) {
        const promptLower = userPrompt.toLowerCase();
        if (promptLower.includes('blocked traffic') || promptLower.includes('denied')) {
            const count = await prisma_1.prisma.firewallLog.count({
                where: { action: { in: ['deny', 'drop', 'block'] } },
            });
            const topBlockedSrc = await prisma_1.prisma.firewallLog.groupBy({
                by: ['srcip'],
                _count: { srcip: true },
                where: { action: { in: ['deny', 'drop', 'block'] } },
        ...
        if (promptLower.includes('vpn')) {
            const vpnLogs = await prisma_1.prisma.firewallLog.findMany({
        ...
        const alertCount = await prisma_1.prisma.alert.count({ where: { status: 'OPEN' } });
        return {
            prompt: userPrompt,
