...
exports.startSyntheticLogGenerator = startSyntheticLogGenerator;
 ...
 */
function startSyntheticLogGenerator(intervalMs = 1000) {
    ...
    setInterval(() => {
        ...
        client.send(message, env_1.env.SYSLOG_UDP_PORT, 'localhost', (err) => {
            if (err) {
                logger_1.logger.error('Error sending synthetic sample log:', err);
            }
        });
        index++;
    }, intervalMs);
    logger_1.logger.info(`Synthetic FortiGate Log Generator running every ${intervalMs}ms targeting UDP localhost:${env_1.env.SYSLOG_UDP_PORT}`);
}
