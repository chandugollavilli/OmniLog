...
class LogRepository {
    static async findLogs(query, organizationId) {
    ...
    static async findById(id) {
        return prisma_1.prisma.firewallLog.findUnique({ where: { id } });
    }
    static async createMany(logs) {
        return prisma_1.prisma.firewallLog.createMany({
            data: logs,
            skipDuplicates: true,
        });
    }
